...
get_obs <- function(max_id){
  ...
  call <- paste('https://api.inaturalist.org/v1/observations?taxon_id=75956&quality_grade=any&identifications=any&per_page=200&order_by=id&order=asc&id_above=',
                max_id, sep="")
...
downlaod_images_hacked <- function(dat, size = "medium", outpath = "/home/millerjared/blue_bsc4892/millerjared/ai-taxonomist/image_outputs/cw-images/") {
...
list <- list.files("/home/millerjared/blue_bsc4892/millerjared/ai-taxonomist/image_outputs/cw-images/")
usable <- vector("character", length = length(list))
ourSpeciesID <- vector("character", length = length(list))
obs_df <- data.frame(obs_id = list, usable = usable, ourSpeciesID)
fwrite(obs_df, "/home/millerjared/blue_bsc4892/millerjared/ai-taxonomist/data/processed/annotated-records/cw-pulled-annotations.csv")
